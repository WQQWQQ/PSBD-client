...
export default {
    ...
    methods: {
        ...
        createGraph2() {
            return new Promise((resolve, reject) => {
                this.$http(getDataTableClassify(), (res) => {
                    let {
                        code,
                        data
                    } = res, graph2DataArr = [], graph2LinksArr = [];
                    if(!code && data) {
                        let {
                            title,
                            num,
        ...
        createGraph3() {
            return new Promise((resolve, reject) => {
                this.$http(getDataClassifyCount(), (res) => {
                    res = res[0] || {};
                    let dataUseArr = res.data || [],
                        dataArr = [],
                        numArr = [];
                    if(!res.code) {
                        let i = dataUseArr.length - 1;
                        for(; i >= 0; i--) {
                            let {
                                dataName,
                                dataSize
        ...
        createGraph4() {
            return new Promise((resolve, reject) => {
                this.$http(getPersonDataCount(), (res) => {
                    res = res[0] || {};
                    let personArr = res.data || [],
                        personArr1 = [],
                        personArr2 = [];
                    if(!res.code) {
                        let i = 0,
        ...
        createGraph6() {
            return new Promise((resolve, reject) => {
                this.$http(getCaseDataCount(), res => {
                    res = res[0] || {};
                    let caseArr = res.data || [],
                        caseArr1 = [],
                        caseArr2 = [];
                    if(!res.code) {
                        let i = 0,
